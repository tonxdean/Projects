...
public:
	...
	*/
	//window is what window number is being traced, the first or the second, 1 or 2
	void raytraceScene(const std::vector<std::shared_ptr<Surface>> & surfaces, const std::vector<std::shared_ptr<LightSource>> & lights,unsigned int aliasing, unsigned int window);
	...
	*/
	//windows is the total number of window, 1 or 2 (only tested for 2)
	void calculatePerspectiveViewingParameters(unsigned int windows, const float verticalFieldOfViewDegrees = 45.0f);
...
protected:
	...
	*/
	...
	color traceIndividualRay(const glm::vec3 &e, const glm::vec3 &d,const std::vector<std::shared_ptr<LightSource>> & lights,int recursionLevel = 0);
